...
class PromptOptimizer(ABC):
    def __init__(self, args, evaluator_fn, scorer, max_threads=1, bf_eval=None, predictor=None):
        self.opt = args
...
class ProTeGi(PromptOptimizer):
    ...
    def _get_gradients(self, prompt, error_string, num_feedbacks=5, n=1):
        ...
        res = self.predictor.eval_multiple(gradient_prompt, n=n)
        feedbacks = []
        new_prompts = []
        for r in res:    
    ...
    def apply_gradient(self, prompt, error_str, feedback_str, steps_per_gradient, n=1):
        ...
        res = self.predictor.eval_multiple(transformation_prompt, n=n)
        new_prompts = []
        for r in res:   
    ...
    def generate_synonyms(self, prompt_section, n=3):
        ...
        new_instructions = self.predictor.eval_multiple(rewriter_prompt, n=n)
    ...
    def expand_candidates(self, prompts, task, gpt4, train_exs):
        ...
        for prompt in tqdm(prompts, desc=f'expanding {len(prompts)} prompts'):
            ...
            _, _, _, texts, labels, preds = task.evaluate(gpt4, prompt, minibatch)
            ...
            new_sections = [sec.replace("{", "").replace("}", "") for sec in new_sections]
            tmp_new_prompts = [
